...
// TestALMBitbucketCRUD creates an ALMBitbucket resource, waits for Ready,
// then verifies SonarQube reports a matching Bitbucket Server ALM
// definition with the expected key and URL.
func TestALMBitbucketCRUD(t *testing.T) {
	t.Parallel()

	f := e2e.New(t)
	const (
		crName         = "e2e-almbitbucket-crud"
		almKey         = "e2e-almbitbucket-crud"
		almURL         = "https://bitbucket.example.com"
		secretName     = "e2e-almbitbucket-crud-token"
		secretKey      = "token"
		connSecretName = "e2e-almbitbucket-crud-connection"
	)

	patSecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: secretName, Namespace: f.Namespace},
		Type:       corev1.SecretTypeOpaque,
		StringData: map[string]string{secretKey: "e2e-fake-bitbucket-pat"},
	}
	...
	alm := &integrationv1alpha1.ALMBitbucket{
		ObjectMeta: metav1.ObjectMeta{Name: crName, Namespace: f.Namespace},
		Spec: integrationv1alpha1.ALMBitbucketSpec{
			...
			ForProvider: integrationv1alpha1.ALMBitbucketParameters{
	...
	got, err := f.FindALMBitbucketDefinitionByKey(context.Background(), almKey)
	if err != nil {
		t.Fatalf("fetching Bitbucket ALM definition: %v", err)
	}
	if got == nil {
		t.Fatalf("Bitbucket ALM definition %q not found in SonarQube", almKey)
	}
	if got.URL != almURL {
