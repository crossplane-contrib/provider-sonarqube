...
// TestALMAzureCRUD creates an ALMAzure resource, waits for Ready, then
// verifies SonarQube reports a matching Azure ALM definition with the
// expected key and URL.
func TestALMAzureCRUD(t *testing.T) {
	t.Parallel()

	f := e2e.New(t)
	const (
		crName         = "e2e-almazure-crud"
		almKey         = "e2e-almazure-crud"
		almURL         = "https://dev.azure.com/example"
		secretName     = "e2e-almazure-crud-token"
		secretKey      = "token"
		connSecretName = "e2e-almazure-crud-connection"
	)

	patSecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: secretName, Namespace: f.Namespace},
		Type:       corev1.SecretTypeOpaque,
		StringData: map[string]string{secretKey: "e2e-fake-azure-pat"},
	}
	...
	alm := &integrationv1alpha1.ALMAzure{
		ObjectMeta: metav1.ObjectMeta{Name: crName, Namespace: f.Namespace},
		Spec: integrationv1alpha1.ALMAzureSpec{
			...
			ForProvider: integrationv1alpha1.ALMAzureParameters{
	...
	got, err := f.FindALMAzureDefinitionByKey(context.Background(), almKey)
	if err != nil {
		t.Fatalf("fetching Azure ALM definition: %v", err)
	}
	if got == nil {
		t.Fatalf("Azure ALM definition %q not found in SonarQube", almKey)
	}
	if got.URL != almURL {
