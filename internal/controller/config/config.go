...
func setupProviderConfig(
	...
	reconciler := providerconfig.NewReconciler(mgr, kinds,
		...
		providerconfig.WithRecorder(event.NewAPIRecorder(mgr.GetEventRecorderFor(name)))) //nolint:deprecated // GetEventRecorderFor is marked as deprecated but is not yet replaced with an alternative in controller-runtime, and the APIRecorder is still required for recording events.
