...
const (
	// testExternalName is the test ALM Bitbucket external name.
	testExternalName = "bitbucket-main"
	// testRenamedBitbucketKey is a renamed Bitbucket key for testing.
	testRenamedBitbucketKey = "bitbucket-renamed"
	// testBitbucketURL is the test Bitbucket URL.
	testBitbucketURL = "https://bitbucket.example.com"
	// bitbucketPATValue is the test Bitbucket personal access token value.
	bitbucketPATValue = "bitbucketPATValue"
)

// notALMBitbucket is a test type that is not an ALMBitbucket.
type notALMBitbucket struct {
	resource.Managed
}
...
// newTestALMBitbucket creates a test ALMBitbucket resource.
func newTestALMBitbucket(externalName string, tokenSelector *xpv1.LocalSecretKeySelector) *v1alpha1.ALMBitbucket {
	alm := &v1alpha1.ALMBitbucket{
		ObjectMeta: metav1.ObjectMeta{
			Name:        "test-alm-bitbucket",
			Namespace:   "default",
			Annotations: map[string]string{},
			UID:         types.UID("alm-bitbucket-uid"),
		},
		Spec: v1alpha1.ALMBitbucketSpec{
			ManagedResourceSpec: xpv1.ManagedResourceSpec{},
			ForProvider: v1alpha1.ALMBitbucketParameters{
				ALMCommonParameters: v1alpha1.ALMCommonParameters{
					URL:                          testBitbucketURL,
...
// TestObserve tests observing ALMBitbucket resource state.
func TestObserve(t *testing.T) {
	...
	cases := map[string]struct {
		...
		settingsClient *fake.MockALMSettingsBitbucketClient
		args           args
		want           want
	}{
		"NotALMBitbucket": {
			objects:        []runtime.Object{},
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
			args:           args{ctx: context.Background(), mg: &notALMBitbucket{}},
			want:           want{observation: managed.ExternalObservation{}, errSubstr: errNotALMBitbucket},
		},
		"MissingExternalNameReturnsNotExists": {
			objects:        []runtime.Object{},
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
			args:           args{ctx: context.Background(), mg: newTestALMBitbucket("", tokenRef)},
			want:           want{observation: managed.ExternalObservation{ResourceExists: false}},
		},
		"MissingPATSelectorReturnsError": {
			objects:        []runtime.Object{},
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
			args:           args{ctx: context.Background(), mg: newTestALMBitbucket(testExternalName, nil)},
		...
		"EmptyPATReturnsError": {
			...
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
			args:           args{ctx: context.Background(), mg: newTestALMBitbucket(testExternalName, tokenRef)},
		...
		"MissingSavedTokenSecretDoesNotFailObserve": {
			objects: []runtime.Object{tokenSecret("pat-secret", "default", "token", bitbucketPATValue)},
			settingsClient: &fake.MockALMSettingsBitbucketClient{ListDefinitionsFn: func() (*sonar.AlmSettingsListDefinitions, *http.Response, error) {
				return &sonar.AlmSettingsListDefinitions{Bitbucket: []sonar.BitbucketDefinition{{Key: testExternalName, URL: testBitbucketURL}}}, mockHTTPResponse(http.StatusOK), nil
			}},
			args: args{ctx: context.Background(), mg: func() resource.Managed {
				alm := newTestALMBitbucket(testExternalName, tokenRef)
				...
				alm.Status.AtProvider.URL = testBitbucketURL
		...
		"MissingSavedTokenKeyDoesNotFailObserve": {
			objects: []runtime.Object{
				tokenSecret("pat-secret", "default", "token", bitbucketPATValue),
		...
		"SuccessfulObserveUpToDate": {
			objects: []runtime.Object{
				tokenSecret("pat-secret", "default", "token", bitbucketPATValue),
				tokenSecret("connection-secret", "default", connectionDetailTokenKey, bitbucketPATValue),
			},
			settingsClient: &fake.MockALMSettingsBitbucketClient{ListDefinitionsFn: func() (*sonar.AlmSettingsListDefinitions, *http.Response, error) {
				return &sonar.AlmSettingsListDefinitions{Bitbucket: []sonar.BitbucketDefinition{{Key: testExternalName, URL: testBitbucketURL}}}, mockHTTPResponse(http.StatusOK), nil
			}},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			e := &external{
				...
				integrationsClient: &fake.MockALMIntegrationsBitbucketClient{},
...
// TestCreate tests creating an ALMBitbucket resource.
func TestCreate(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"NotALMBitbucket": {
			...
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
			args:           args{ctx: context.Background(), mg: &notALMBitbucket{}},
			want:           want{creation: managed.ExternalCreation{}, errSubstr: errNotALMBitbucket},
		},
		"PATReadError": {
			objects:        []runtime.Object{},
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
			args:           args{ctx: context.Background(), mg: newTestALMBitbucket("", nil)},
		...
		"CreateError": {
			objects: []runtime.Object{tokenSecret("pat-secret", "default", "token", bitbucketPATValue)},
			settingsClient: &fake.MockALMSettingsBitbucketClient{
				CreateBitbucketFn: func(_ *sonar.AlmSettingsCreateBitbucketOptions) (*http.Response, error) {
			...
			args: args{ctx: context.Background(), mg: newTestALMBitbucket("", tokenRef)},
			want: want{creation: managed.ExternalCreation{}, errSubstr: "cannot create ALMBitbucket resource in SonarQube API"},
		},
		"SuccessfulCreate": {
			objects: []runtime.Object{tokenSecret("pat-secret", "default", "token", bitbucketPATValue)},
			settingsClient: &fake.MockALMSettingsBitbucketClient{
				CreateBitbucketFn: func(opt *sonar.AlmSettingsCreateBitbucketOptions) (*http.Response, error) {
					if opt == nil || opt.Key != testExternalName || opt.URL != testBitbucketURL || opt.PersonalAccessToken != bitbucketPATValue {
			...
			args: args{ctx: context.Background(), mg: newTestALMBitbucket("", tokenRef)},
			want: want{creation: managed.ExternalCreation{ConnectionDetails: managed.ConnectionDetails{connectionDetailTokenKey: []byte(bitbucketPATValue)}}},
		},
	}
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			alm, ok := tc.args.mg.(*v1alpha1.ALMBitbucket)
...
// TestUpdate tests updating an ALMBitbucket resource.
func TestUpdate(t *testing.T) { //nolint:gocognit // table-driven test covers all update paths
	...
	cases := map[string]struct {
		...
		"NotALMBitbucket": {
			...
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
			args:           args{ctx: context.Background(), mg: &notALMBitbucket{}},
			want:           want{update: managed.ExternalUpdate{}, errSubstr: errNotALMBitbucket},
		},
		"MissingExternalName": {
			objects:        []runtime.Object{},
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
		...
		"UpdateError": {
			objects: []runtime.Object{tokenSecret("pat-secret", "default", "token", bitbucketPATValue)},
			settingsClient: &fake.MockALMSettingsBitbucketClient{
				UpdateBitbucketFn: func(_ *sonar.AlmSettingsUpdateBitbucketOptions) (*http.Response, error) {
					//nolint:nilnil // Intentional: simulating partial HTTP failure.
					return mockHTTPResponse(http.StatusInternalServerError), errors.New("api update failed")
				},
			},
			args: args{ctx: context.Background(), mg: newTestALMBitbucket(testExternalName, tokenRef)},
			want: want{update: managed.ExternalUpdate{}, errSubstr: "cannot update ALMBitbucket resource in SonarQube API"},
		},
		"SuccessfulUpdateWithoutKeyChange": {
			objects: []runtime.Object{tokenSecret("pat-secret", "default", "token", bitbucketPATValue)},
			settingsClient: &fake.MockALMSettingsBitbucketClient{
				UpdateBitbucketFn: func(opt *sonar.AlmSettingsUpdateBitbucketOptions) (*http.Response, error) {
					if opt == nil || opt.Key != testExternalName || opt.NewKey != "" || opt.URL != testBitbucketURL || opt.PersonalAccessToken != bitbucketPATValue {
			...
			args: args{ctx: context.Background(), mg: newTestALMBitbucket(testExternalName, tokenRef)},
			want: want{update: managed.ExternalUpdate{ConnectionDetails: managed.ConnectionDetails{connectionDetailTokenKey: []byte(bitbucketPATValue)}}},
		},
		"SuccessfulUpdateWithKeyChange": {
			objects: []runtime.Object{tokenSecret("pat-secret", "default", "token", bitbucketPATValue)},
			settingsClient: &fake.MockALMSettingsBitbucketClient{
				UpdateBitbucketFn: func(opt *sonar.AlmSettingsUpdateBitbucketOptions) (*http.Response, error) {
					if opt == nil || opt.Key != testExternalName || opt.NewKey != testRenamedBitbucketKey {
						t.Fatalf("Update() unexpected options: %+v", opt)
					}
			...
			args: args{ctx: context.Background(), mg: func() resource.Managed {
				alm := newTestALMBitbucket(testExternalName, tokenRef)
				alm.Spec.ForProvider.Key = testRenamedBitbucketKey
		...
		"UpdateKeyChangePersistError": {
			...
			// registerMG is false → ALMBitbucket not in fake client → kubeClient.Update returns not-found
			args: args{ctx: context.Background(), mg: func() resource.Managed {
				alm := newTestALMBitbucket(testExternalName, tokenRef)
				alm.Spec.ForProvider.Key = testRenamedBitbucketKey
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.registerMG {
				if alm, ok := tc.args.mg.(*v1alpha1.ALMBitbucket); ok {
			...
			if alm, ok := tc.args.mg.(*v1alpha1.ALMBitbucket); ok && name == "SuccessfulUpdateWithKeyChange" {
				if gotExternalName := meta.GetExternalName(alm); gotExternalName != testRenamedBitbucketKey {
					t.Fatalf("Update() external name = %q, want %q", gotExternalName, testRenamedBitbucketKey)
				}

				// Verify the external name was persisted to the Kubernetes API.
				persisted := &v1alpha1.ALMBitbucket{}
				...
				if gotPersistedName := meta.GetExternalName(persisted); gotPersistedName != testRenamedBitbucketKey {
					t.Fatalf("Update() persisted external name = %q, want %q", gotPersistedName, testRenamedBitbucketKey)
				}
			}
		})
	}
...
// TestDelete tests deleting an ALMBitbucket resource.
func TestDelete(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"NotALMBitbucket": {
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
			args:           args{ctx: context.Background(), mg: &notALMBitbucket{}},
			want:           want{deletion: managed.ExternalDelete{}, errSubstr: errNotALMBitbucket},
			wantCall:       false,
		},
		"MissingExternalName": {
			settingsClient: &fake.MockALMSettingsBitbucketClient{},
		...
		"DeleteError": {
			...
			args:     args{ctx: context.Background(), mg: newTestALMBitbucket(testExternalName, nil)},
			want:     want{deletion: managed.ExternalDelete{}, errSubstr: "cannot delete ALMBitbucket resource in SonarQube API"},
			wantCall: true,
		},
		"SuccessfulDelete": {
			settingsClient: &fake.MockALMSettingsBitbucketClient{
...
// TestDisconnect tests disconnecting an ALMBitbucket resource.
func TestDisconnect(t *testing.T) {
	t.Parallel()

	e := &external{integrationsClient: &fake.MockALMIntegrationsBitbucketClient{}, settingsClient: &fake.MockALMSettingsBitbucketClient{}}
...
func TestGetSavedAPIToken(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"WriteSecretNameEmpty": {
			objects: nil,
			alm: func() *v1alpha1.ALMBitbucket {
				alm := newTestALMBitbucket(testExternalName, tokenRef)
...
// TestConnect tests connecting to an ALMBitbucket resource.
func TestConnect(t *testing.T) {
	...
	t.Run("TypeAssertionError", func(t *testing.T) {
		...
		_, err := c.Connect(context.Background(), &notALMBitbucket{})
		if err == nil || !strings.Contains(err.Error(), errNotALMBitbucket) {
			t.Fatalf("Connect() error = %v, want to contain %q", err, errNotALMBitbucket)
		}
	})
	...
	t.Run("TrackUsageError", func(t *testing.T) {
		...
		alm := newTestALMBitbucket("bitbucket-main", &xpv1.LocalSecretKeySelector{LocalSecretReference: xpv1.LocalSecretReference{Name: "pat-secret"}, Key: "token"})
...
// TestSetupGatedRegistersALMBitbucketGVK tests that SetupGated
// registers the ALMBitbucket GVK.
func TestSetupGatedRegistersALMBitbucketGVK(t *testing.T) {
	t.Parallel()
	...
	if diff := cmp.Diff(v1alpha1.ALMBitbucketGroupVersionKind, g.gvks[0]); diff != "" {
